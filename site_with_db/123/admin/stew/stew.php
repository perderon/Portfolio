...
<h2 class="main-content">Стюардессы</h2>
<body>
    <table>
        <tr>
            <th>Номер</th>
        ...
        <?php
            ...
            $sql = "SELECT * FROM stewardess";
            $result = mysqli_query($conn, $sql);

            if (mysqli_num_rows($result) > 0) {
                while($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td>" . $row["idstew"] . "</td>";
                    echo "<td>" . $row["FN"] . "</td>";
                    echo "<td>" . $row["SN"] . "</td>";
                    echo "<td>" . $row["DN"] . "</td>";
                    echo "<td>" . $row["BD"] . "</td>";
                    ...
                    echo "<td>
                              <a href='edit_stew.php?idstew=" . $row["idstew"] . "' class='btn btn-info btn-sm'><i class='fa fa-pencil'></i> Изменить</a>
                              <a href='delete_stew.php?idstew=" . $row["idstew"] . "' class='btn btn-danger btn-sm'><i class='fa fa-trash'></i> Удалить</a>
                            </td>";
                      echo "</tr>";
                }
            } else {
    ...
    <form action="insert_stew.php" method="POST">
